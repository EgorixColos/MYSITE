...
Источник данных - это объект или система откуда поступают данные для обработки. Приеник данных - это объект или система куда передаются обработанные данные.
Примеры источников:
1. Базы данных (SQL или NoSQL)
2. Файлы
3. API
4. Потоки данных
...
2. Отчеты и вызуализации
3. Внешнии системы 
4. Облачные хранилища 

Критерии выбора источников и приемника.
При выборе учитывают:
- Формат данных (стуктурированных, полуструктурированный и неструктурированный)
- Объем данных (большие данные требуют распределенных систем)
- Частота обновления (Real time, пакетная обработка) 
- Надежность и доступность
- Безопасность (можно обеспечить с помощью шифрования, либо доступа по ролям)
- Стоимость (лицензии и облачные тарифы)

## Сопоставление объектов данных.
Процесс преобразования данных между источников и приемником 

Этапы сопоставления:
    1. Анализ структуры данных (сравнение полей в источнике и приемнике)
    2. Преобразование типов 
Например: Строку в дату, либо из даты в строку (  | 15.09.2025 -> "15.09.2025" |  )
    3. Маппинг полей (сопоставление названий столбцов: | user_id -> БЭК --> client_id -> фронт |  )
Существуют различного рода интсрументы для маппинга
    4. Обработка отсутсвующих данных (При отсутствии данных поля заполняются, либо дефолтными значениями, либо явно указанными)
    5. Валидация - это проверка на корректность 

## Проблемы и решения.
    1. Не совпадение схем (испольование унифицированных форматов)
    2. Потери данных (на помощь приходил логированние и мониторинг)
    3. Производительность (оптимизируются запросы, индексы и реализуется кэширование)


# Транспортные протоколы. Стандарты формирования сообщения.
Транспортые протоколы определяют правила передачи данных между системами. 
Основные транспортные протоколы:
1. Http и Https
    - Http - hyper text transfer протокол. Протокол для передачи данных в веб среде (незащищен)
    - Https - Http secure - защищенная версия с шифрованием SSl и TLS
...
- Использование: чаты, онлайн игры, бирхевые котировки.

3. TCP/IP и UDP
- TCP (Transmission Control Protocol) надежная передача с продтверждением (гарантирует доставку)
...
4. MQTT (Message Queuing Telemetry Transpost)
...
- Высокопроизврдительный PPC-протокол на основе HTTP/2.
